        ...
        randomWordUrl : (data) => {
            return new Promise((resolve, reject) => {
                if(data) {
                    ...
                    randomNum = 3;
                    const urlPage = $('.concept_light').eq(randomNum-1).find('a.light-details_link').attr('href');
            ...
            return new Promise((resolve, reject) => {
                if(data) {
                    ...
                    const words = $('.concept_light').each((i, word) => {
                        ...
                        const kanji = $(word).children('.concept_light-wrapper').find('span.text').text().trim();
                        const furigana = $(word).children('.concept_light-wrapper').find('span.furigana').text().trim();
                            console.log(i,"furigana: " + furigana);
                            console.log("kanji: " + kanji);
                        
                        const jlptLevel = $(word).children('.concept_light-wrapper').find('span.concept_light-tag').next().html();
                            console.log("jlpt Level: " + jlptLevel);

                        const typeOfWord = $(word).children('.concept_light-meanings').find('.meaning-tags').html();
